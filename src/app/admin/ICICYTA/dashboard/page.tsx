...
export default function ICICYTADashboardPage() {
     ...
     useEffect(() => {
          if (!authLoading && (!user || (user.role !== 'ADMIN_ICICYTA' && user.role !== 'SUPER_ADMIN'))) {
               // Redirect if not authorized (allowing super admin too for preview)
               if (!user) router.push("/login");
               // If logged in but wrong role, maybe redirect to home or unauthorized
          }
               ...
               <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4">
                    <div>
                         <h1 className="text-3xl font-bold text-[#015B97]">
                              ICICyTA Admin Dashboard
                         </h1>
                         <p className="text-gray-500 mt-1">Manage your ICICyTA conference events and schedules.</p>
                    </div>
                    <Button onClick={() => router.push('/admin/ICICYTA')} className="bg-[#015B97] hover:bg-[#014f7a]">
                              ...
                              <div key={conf.id} className="grid grid-cols-12 gap-4 p-4 items-center hover:bg-gray-50 transition-colors">
                                   ...
                                   <div className="col-span-3 text-right">
                                        <Button
                                             ...
                                             onClick={() => router.push(`/admin/ICICYTA?year=${conf.year}`)}
                                        >
